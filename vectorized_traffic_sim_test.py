...
#     speeds = [10,2,1,1]
#     loop_length = 60
#     print(are_too_close(fronts, lengths, speeds, loop_length))
#     assert np.array_equal(are_too_close(fronts, lengths, speeds, loop_length),[[1],[],[],[]])
#     #assert np.array_equal(are_too_close(fronts, lengths, speeds, loop_length),[True, False, False, False])
#
# def test_are_too_close_2():
#     fronts = np.array([0,10,20,50])
#     lengths = np.array([5,5,5,5])
#     speeds = [10,2,100,1]
#     loop_length = 60
#     # print(are_too_close(fronts, lengths, speeds, loop_length))
#     # assert False
#     assert np.array_equal(are_too_close(fronts, lengths, speeds, loop_length),[[1],[],[0,1,2,3],[]])
#     #assert np.array_equal(are_too_close(fronts, lengths, speeds, loop_length),[True, False, False, False])
#
...
#     assert np.array_equal(speed_up(speeds,should_speed_up,target_speeds,accelerations),[2,10,21,33])
#
# def test_stop_vehicles():
#     speeds = [0, 10, 20, 40]
#     will_stop = [False, True, False, False]
#     assert np.array_equal(stop_vehicles(speeds,will_stop),[0,0,20,40])
#
# def test_sim_tick():
#     fronts=np.array([0,10,20])
#     lengths = np.array([5,5,5])
#     speeds = np.array([6,0,10])
#     accelerations = np.array([2,2,2])
#     target_speeds = np.array([33,33,33])
#     loop_length =100
#     sim_tick(fronts, lengths, speeds, accelerations, target_speeds, loop_length)
#     assert False
#
